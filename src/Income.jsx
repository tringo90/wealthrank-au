...
const ATO_ALL={label:"All Australian earners",percentiles:{10:14000,20:24000,30:35000,40:48000,50:62400,60:78000,70:95000,80:118000,90:155000,95:200000,99:350000},median:62400};
const ATO_AGE={"18-24":{label:"Ages 18–24",percentiles:{10:6000,20:12000,30:18000,40:24000,50:30000,60:38000,70:48000,80:62000,90:82000,95:105000,99:165000},median:30000},"25-34":{label:"Ages 25–34",percentiles:{10:18000,20:32000,30:46000,40:60000,50:74000,60:90000,70:108000,80:130000,90:165000,95:210000,99:360000},median:74000},"35-44":{label:"Ages 35–44",percentiles:{10:18000,20:32000,30:48000,40:64000,50:80000,60:98000,70:118000,80:145000,90:190000,95:245000,99:420000},median:80000},"45-54":{label:"Ages 45–54",percentiles:{10:16000,20:30000,30:46000,40:62000,50:78000,60:96000,70:116000,80:142000,90:185000,95:240000,99:410000},median:78000},"55-64":{label:"Ages 55–64",percentiles:{10:12000,20:24000,30:38000,40:54000,50:68000,60:84000,70:102000,80:126000,90:165000,95:215000,99:380000},median:68000},"65+":{label:"Ages 65+",percentiles:{10:8000,20:18000,30:28000,40:38000,50:48000,60:60000,70:74000,80:92000,90:125000,95:165000,99:310000},median:48000}};
const ATO_STATE={NSW:{label:"New South Wales",median:68000,top10:168000},VIC:{label:"Victoria",median:65000,top10:160000},QLD:{label:"Queensland",median:62000,top10:155000},WA:{label:"Western Australia",median:70000,top10:175000},SA:{label:"South Australia",median:59000,top10:148000},TAS:{label:"Tasmania",median:56000,top10:140000},ACT:{label:"ACT",median:82000,top10:195000},NT:{label:"Northern Territory",median:72000,top10:180000}};
const LBLS=[{min:0,max:25,label:"Entry level",color:"#6B8CAE"},{min:25,max:50,label:"Middle income",color:"#5BA08A"},{min:50,max:75,label:"Above average",color:"#E8935A"},{min:75,max:90,label:"High income",color:"#E8C05A"},{min:90,max:100,label:"Top earner",color:"#C9A84C"}];

function getAgeGroup(a){const n=parseInt(a);if(n<=24)return"18-24";if(n<=34)return"25-34";if(n<=44)return"35-44";if(n<=54)return"45-54";if(n<=64)return"55-64";return"65+";}
function calcPct(inc,ds){const p=ds.percentiles,k=[10,20,30,40,50,60,70,80,90,95,99];if(inc<=0)return 2;if(inc<p[10])return Math.round((inc/p[10])*10);for(let i=0;i<k.length-1;i++){const lo=k[i],hi=k[i+1];if(inc>=p[lo]&&inc<p[hi])return Math.round(lo+((inc-p[lo])/(p[hi]-p[lo]))*(hi-lo));}return Math.round(99+Math.min((inc-p[99])/p[99],1)*0.9);}
...
function Bell({pct,color,animated}){
  ...
  useEffect(()=>{
    ...
    ctx.beginPath();for(let i=0;i<=n;i++){const x=(i/n)*(W-60)+30,t=i/n,y=H-30-(g(t)/my)*(H-60);if(i===0)ctx.moveTo(x,H-30);ctx.lineTo(x,y);if(x>=mX)break;}
      ...
      <div style={{maxWidth:780,margin:"0 auto"}}>
        ...
        <div style={{borderTop:"1px solid rgba(240,237,230,0.05)",paddingTop:16,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{fontSize:10,color:"rgba(240,237,230,0.18)"}}>Data: ATO 2021–22 · ABS 2021–22</div>
...
export default function Income({setPage}){
  const [form,setForm]=useState({income:"",age:"",state:"",compareMode:"age"});
  ...
  const calc=()=>{
    ...
    const ageData=ATO_AGE[ageGrp],allData=ATO_ALL;
    const agePct=calcPct(income,ageData),allPct=calcPct(income,allData);
    ...
    const weekly=Math.round(income/52),monthly=Math.round(income/12);
    let tax=0;
    ...
    const afterTax=Math.max(0,income-tax-income*0.02);
  ...
  const rcm=(m,r)=>{
    const dataset=m==="age"?r.ageData:r.allData;
    const pct=m==="age"?r.agePct:r.allPct;
    sf("compareMode",m);setResult({...r,pct,dataset});
    setAnimated(false);setTimeout(()=>setAnimated(true),50);
  };

  const share=()=>{
    const txt=`I'm in the top ${100-result?.pct}% of Australian earners. Where do you rank?`;
  ...
  const tg=(active)=>({flex:1,padding:"10px",border:"none",background:active?accent:"transparent",color:active?"#0D1B2A":"rgba(240,237,230,0.5)",fontWeight:active?700:400,cursor:"pointer",fontSize:12,transition:"all 0.15s"});
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0D1B2A",color:"#F0EDE6",fontFamily:"'Inter',system-ui,sans-serif"}}>
      ...
      <div style={{textAlign:"center",padding:"52px 24px 40px",borderBottom:"1px solid rgba(240,237,230,0.06)"}}>
        ...
        <h1 style={{fontSize:"clamp(24px,5vw,48px)",fontWeight:800,lineHeight:1.05,letterSpacing:"-0.02em",margin:"0 0 10px",color:"#F0EDE6"}}>Income Percentile Calculator</h1>
      ...
      <div style={{maxWidth:580,margin:"0 auto",padding:"32px 24px 0"}}>
        {!result ? (
          <div style={{background:"#142133",border:"1px solid rgba(240,237,230,0.08)",borderRadius:16,padding:"26px"}}>
            ...
            <button style={{width:"100%",padding:"14px",background:accent,color:"#0D1B2A",border:"none",borderRadius:10,fontSize:15,fontWeight:700,cursor:"pointer"}} onClick={calc}>
              Calculate My Rank →
            </button>
          </div>
        ) : (
          <div ref={resultRef}>
            <div style={{textAlign:"center",padding:"36px 0 22px"}}>
              ...
              <div style={{fontSize:15,color:"rgba(240,237,230,0.6)",marginTop:4,marginBottom:22}}>
                percentile — earning more than <strong style={{color:"#F0EDE6"}}>{result.pct}%</strong> of {form.compareMode==="age"?`Australian earners aged ${result.ageData.label.toLowerCase()}`:"all Australian earners"}
              </div>
            </div>
            <div style={{background:"#142133",border:"1px solid rgba(240,237,230,0.07)",borderRadius:14,padding:"20px 12px 8px",marginBottom:16}}>
              <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:"rgba(240,237,230,0.28)",marginBottom:8,paddingLeft:4}}>Income Distribution</div>
              <Bell pct={result.pct} animated={animated} color={accent}/>
            </div>
            <div style={{display:"flex",gap:8,marginBottom:16}}>
              {["age","all"].map(m=>(
                <button key={m} onClick={()=>rcm(m,result)} style={{flex:1,padding:"8px",border:`1px solid ${form.compareMode===m?accent+"88":"rgba(240,237,230,0.09)"}`,background:form.compareMode===m?accent+"14":"transparent",color:form.compareMode===m?accent:"rgba(240,237,230,0.38)",borderRadius:8,fontSize:12,fontWeight:form.compareMode===m?700:400,cursor:"pointer"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
              {[
                ...
                {label:"Est. after-tax (pa)",value:fmt(result.afterTax),color:"#7EB8D4"},
              ].map(s=>(
            ...
            <div style={{background:"linear-gradient(135deg,#1a2e42,#142133)",border:`1px solid ${accent}35`,borderRadius:12,padding:"18px",marginBottom:16}}>
              ...
              <div style={{fontSize:13,color:"rgba(240,237,230,0.58)",lineHeight:1.7}}>A high income is only part of the picture. Wealth is built through what you save and invest. See how your <strong style={{color:"#F0EDE6"}}>net worth</strong> compares against other Australians.</div>
              <button onClick={()=>setPage("calculator")} style={{marginTop:12,padding:"9px 18px",background:accent,color:"#0D1B2A",border:"none",borderRadius:8,fontSize:13,fontWeight:700,cursor:"pointer"}}>Calculate Net Worth Rank →</button>
            </div>
            ...
            <button style={{width:"100%",padding:"10px",background:"transparent",color:"rgba(240,237,230,0.3)",border:"1px solid rgba(240,237,230,0.09)",borderRadius:10,fontSize:12,cursor:"pointer",marginBottom:32}} onClick={()=>setResult(null)}>← Recalculate</button>
            <div style={{textAlign:"center",fontSize:9,color:"rgba(240,237,230,0.16)",marginBottom:32,letterSpacing:"0.05em"}}>DATA: ATO 2021–22 · FOR INFORMATIONAL USE ONLY · NOT FINANCIAL ADVICE</div>
          </div>
        )}
      </div>
