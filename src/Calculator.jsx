...
const AF=[{key:"property",label:"Property",hint:"Home + investment"},{key:"super",label:"Super",hint:"Combined balance"},{key:"shares",label:"Shares/ETFs",hint:"ASX, funds, crypto"},{key:"cash",label:"Cash",hint:"Bank, deposits"},{key:"vehicle",label:"Vehicles",hint:"Cars, boats"},{key:"other_a",label:"Other Assets",hint:"Business, collectibles"}];
const LF=[{key:"mortgage",label:"Mortgage",hint:"Outstanding"},{key:"hecs",label:"HECS",hint:"Study debt"},{key:"carloan",label:"Car Loan",hint:"Finance"},{key:"creditcard",label:"Credit Cards",hint:"Outstanding"},{key:"personal",label:"Personal Loans",hint:"Unsecured"},{key:"other_l",label:"Other",hint:"Tax debt, BNPL"}];
...
function Bell({percentile,animated,color}){
  ...
  useEffect(()=>{
    ...
    ctx.beginPath();ctx.moveTo(mX,H-30);ctx.lineTo(mX,mY-18);ctx.strokeStyle=color;ctx.lineWidth=2;ctx.setLineDash([4,3]);ctx.stroke();ctx.setLineDash([]);
    const pw=32,ph=18,px=mX-pw/2,py=mY-18-ph-4;
    ctx.beginPath();ctx.moveTo(px+4,py);ctx.lineTo(px+pw-4,py);ctx.arcTo(px+pw,py,px+pw,py+4,4);ctx.lineTo(px+pw,py+ph-4);ctx.arcTo(px+pw,py+ph,px+pw-4,py+ph,4);ctx.lineTo(px+4,py+ph);ctx.arcTo(px,py+ph,px,py+ph-4,4);ctx.lineTo(px,py+4);ctx.arcTo(px,py,px+4,py,4);ctx.closePath();
    ctx.fillStyle=color;ctx.fill();
    ctx.fillStyle="#0D1B2A";ctx.font="bold 9px system-ui";ctx.textAlign="center";ctx.fillText("YOU",mX,py+12);
    ctx.beginPath();ctx.arc(mX,mY-18,5,0,Math.PI*2);ctx.fillStyle=color;ctx.fill();
    ctx.beginPath();ctx.arc(mX,mY-18,2.5,0,Math.PI*2);ctx.fillStyle="#fff";ctx.fill();
  },[dp,color]);
...
function WhatIf({nw,accent}){
  ...
  return(
    <div style={{background:"#0f1e2d",border:"1px solid rgba(240,237,230,0.07)",borderRadius:16,padding:"20px",marginBottom:16}}>
      <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:"rgba(240,237,230,0.3)",marginBottom:14}}>What-If Simulator</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
        {[10,20,30].map(y=>{const v=proj(y),h=Math.round((v/mx)*64);return(
          <div key={y} style={{background:"rgba(13,27,42,0.6)",borderRadius:10,padding:"12px 8px",textAlign:"center"}}>
            ...
            <div style={{fontSize:13,fontWeight:700,color:"#F0EDE6"}}>{fmt(v)}</div>
...
export default function Calculator({setPage}){
  ...
  const share=()=>{
    const txt=`I'm in the top ${100-res?.pct}% of Australian ${ic?"couples":"individuals"} by net worth. Where do you rank?`;
    if(navigator.share)navigator.share({title:"My Wealth Rank",text:txt});
  ...
  const seg=(active,c)=>({flex:1,padding:"10px",border:"none",background:active?c+"18":"transparent",color:active?c:"rgba(240,237,230,0.4)",fontWeight:active?700:400,cursor:"pointer",fontSize:13,borderBottom:`2px solid ${active?c:"transparent"}`,transition:"all 0.15s"});
    ...
    <div style={{minHeight:"100vh",background:"#0D1B2A",color:"#F0EDE6",fontFamily:"'Inter',system-ui,sans-serif"}}>
      ...
      <div style={{borderBottom:"1px solid rgba(240,237,230,0.06)",padding:"44px 24px 36px"}}>
        <div style={{maxWidth:900,margin:"0 auto"}}>
          ...
          <h1 style={{fontSize:"clamp(28px,5vw,52px)",fontWeight:900,lineHeight:1.05,letterSpacing:"-0.025em",margin:"0 0 12px",color:"#F0EDE6"}}>Your Financial Position</h1>
          <p style={{fontSize:15,color:"rgba(240,237,230,0.5)",maxWidth:560,margin:0,lineHeight:1.7}}>See where your household net worth sits relative to other Australians, based on ABS data.</p>
        </div>
      </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"340px 1fr",gap:32,alignItems:"start"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:400,textAlign:"center",padding:"48px 24px"}}>
              <div style={{width:80,height:80,background:"rgba(232,147,90,0.06)",border:"2px dashed rgba(232,147,90,0.2)",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:20}}>
                <div style={{fontSize:28,opacity:0.4,color:"#F0EDE6"}}>%</div>
              </div>
              <div style={{fontSize:16,fontWeight:700,color:"rgba(240,237,230,0.5)",marginBottom:8}}>Your result will appear here</div>
              <div style={{fontSize:13,color:"rgba(240,237,230,0.3)",lineHeight:1.6,maxWidth:280}}>Enter your age, assets, and liabilities on the left to see where you rank.</div>
            </div>
          </div>
        ) : (
          <div ref={resultRef}>
            ...
            <div style={{textAlign:"center",marginBottom:32}}>
              ...
              <div style={{fontSize:16,color:"rgba(240,237,230,0.55)"}}>
                Percentile — ahead of <strong style={{color:"#F0EDE6"}}>{res.pct}%</strong> of {form.cmp==="age"?`Australian ${sub}s aged ${res.ad.label.toLowerCase()}`:`all Australian ${sub}s`}
              </div>
            </div>
            <div style={{background:"#142133",border:"1px solid rgba(240,237,230,0.07)",borderRadius:16,padding:"24px 16px 12px",marginBottom:16}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,padding:"0 4px"}}>
                <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:"rgba(240,237,230,0.3)"}}>Percentile Ranking</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16}}>
              {[
                {label:"Your Net Worth",value:`${res.nw<0?"-":""}${fmt(Math.abs(res.nw))}`,color:"#F0EDE6"},
                {label:"Median Benchmark",value:fmt(res.ds.median),color:"rgba(240,237,230,0.55)"},
                {label:"Total Assets",value:fmt(res.a),color:"#5BA08A"},
                {label:"Total Liabilities",value:fmt(res.l),color:"#E8935A"},
              ].map(s=>(
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:12,marginBottom:16,alignItems:"stretch"}}>
              {res.ms&&(
                <div style={{background:"linear-gradient(135deg,#1a2e42,#142133)",border:`1px solid ${acc}30`,borderRadius:14,padding:"20px"}}>
                  <div style={{fontSize:9,letterSpacing:"0.1em",textTransform:"uppercase",color:"rgba(240,237,230,0.3)",marginBottom:4}}>Next Milestone</div>
                  <div style={{fontSize:17,fontWeight:800,marginBottom:4,color:"#F0EDE6"}}>{res.ms.p}th Percentile</div>
